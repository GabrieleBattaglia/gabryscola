...
class IAPerfetta:
    ...
    @classmethod
    def _minimax(cls, mano_pc, mano_umano, briscola_seme, turno_pc, punti_pc, punti_umano, depth, alpha, beta):
        ...
        if turno_pc:
            for c_pc in mano_pc:
                ...
                for c_umano in mano_umano:
                    vincitore = cls._determina_vincitore_mano_logica(c_pc, c_umano, briscola_seme)
                    punti_presi = cls._punti(c_pc) + cls._punti(c_umano)
                    
                    nuovi_punti_pc = punti_pc + punti_presi if vincitore == 1 else punti_pc
                    nuovi_punti_umano = punti_umano + punti_presi if vincitore == 2 else punti_umano
                    nuova_mano_pc = [c for c in mano_pc if c != c_pc]
                    nuova_mano_umano = [c for c in mano_umano if c != c_umano]
                    nuovo_turno_pc = (vincitore == 1)
                    
                    score = cls._minimax(nuova_mano_pc, nuova_mano_umano, briscola_seme, nuovo_turno_pc, nuovi_punti_pc, nuovi_punti_umano, depth-1, alpha, beta)
            ...
            for c_umano in mano_umano:
                ...
                for c_pc in mano_pc:
                    vincitore = cls._determina_vincitore_mano_logica(c_umano, c_pc, briscola_seme)
                    punti_presi = cls._punti(c_umano) + cls._punti(c_pc)
                    
                    nuovi_punti_pc = punti_pc + punti_presi if vincitore == 2 else punti_pc
                    nuovi_punti_umano = punti_umano + punti_presi if vincitore == 1 else punti_umano
                    nuova_mano_pc = [c for c in mano_pc if c != c_pc]
                    nuova_mano_umano = [c for c in mano_umano if c != c_umano]
    ...
    @classmethod
    def _risolvi_endgame(cls, mano_pc, carte_umano, tavolo, briscola_seme, punti_pc, punti_umano, turno_pc):
        if tavolo:
            ...
            for c_pc in mano_pc:
                vincitore = cls._determina_vincitore_mano_logica(carta_avversario, c_pc, briscola_seme)
                ...
                score = cls._minimax(nuova_mano_pc, nuova_mano_umano, briscola_seme, nuovo_turno_pc, nuovi_punti_pc, nuovi_punti_umano, len(nuova_mano_pc)*2, -float('inf'), float('inf'))
    ...
    @classmethod
    def scegli_carta(cls, briscola, tavolo, mano_pc, mazzo_completo, carte_uscite, punti_pc, punti_umano):
        is_briscola = lambda c: c.seme_nome == briscola.seme_nome
        
        carte_incognite = list(set(mazzo_completo) - carte_uscite - set(mano_pc))
        
        if len(carte_incognite) <= 3 and len(mano_pc) == len(carte_incognite) and len(carte_incognite) > 0:
            turno_pc = len(tavolo) == 0
            carta_scelta = cls._risolvi_endgame(mano_pc, carte_incognite, tavolo, briscola.seme_nome, punti_pc, punti_umano, turno_pc)
            if carta_scelta:
                return carta_scelta
        ...
        if tavolo:
            ...
            for carta_da_giocare in mano_pc:
                vincitore_idx = cls._determina_vincitore_mano_logica(carta_avversario, carta_da_giocare, briscola.seme_nome)
                ...
                if vincitore == "PC":
                    if is_briscola(carta_da_giocare) and not is_briscola(carta_avversario) and punti_mano < 10:
                        valore -= 20
                else:
            ...
            for carta_da_giocare in mano_pc:
                ...
                if num_carte_incognite > 0:
                    for carta_avv_potenziale in carte_incognite:
                        vincitore_idx = cls._determina_vincitore_mano_logica(carta_da_giocare, carta_avv_potenziale, briscola.seme_nome)
                        vincitore = "PC" if vincitore_idx == 1 else "UMANO"
                        punti_mano = punti_da_rischiare + cls._punti(carta_avv_potenziale)
                        if vincitore == "PC":
                ...
                if is_briscola(carta_da_giocare) and punti_da_rischiare > 3:
                    costo_opportunita = (punti_da_rischiare + 5) * (len(carte_incognite) / 10)
