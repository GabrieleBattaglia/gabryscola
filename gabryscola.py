...
from gestione_dati import load_classifica, update_and_display_classifica, save_classifica

def stampa_riepilogo_match(gioco, risultati, punti_totali, numero_partite_match, partite_giocate):
    ...
    print(f"Partite giocate: {partite_giocate} di {numero_partite_match}. Partite rimanenti: {partite_rimanenti}.")
    ...
    if punti_mancanti_umano <= 0:
        ...
        if vittorie_necessarie > partite_rimanenti:
            ...
            print(f"  Situazione critica: devi vincerle tutte! Ti servono {vittorie_necessarie} vittorie su {partite_rimanenti} partite.")
        else:
            print(f"  Per vincere ti servono ancora {punti_mancanti_umano} punti. Un possibile percorso è vincere {vittorie_necessarie} delle prossime {partite_rimanenti} partite.")
    ...
    if punti_mancanti_pc <= 0:
        ...
        if vittorie_necessarie_pc > partite_rimanenti:
            ...
            print(f"  Gli servono ancora {punti_mancanti_pc} punti. Deve vincere almeno {vittorie_necessarie_pc} delle prossime {partite_rimanenti} partite.")
    print("------------------------------------")

def avvia_match(gioco, numero_partite_match):
    print(f"Benvenuto a Gabryscola v{MotoreBriscola.VERSIONE}")
    print(f"Oggi {gioco.giocatore_umano.nome} sfida {gioco.giocatore_pc.nome} in un match al meglio di {numero_partite_match} partite!")
    ...
    for i in range(numero_partite_match):
        ...
        vincitore_partita, punti_u, punti_pc = gioco.gioca_partita(giocatore_di_mano_corrente)
        ...
        if vincitore_partita:
            ...
            perdente_nome = gioco.giocatore_pc.nome if vincitore_partita == gioco.giocatore_umano else gioco.giocatore_umano.nome
        ...
        score_umano = risultati[gioco.giocatore_umano.nome]
        score_pc = risultati[gioco.giocatore_pc.nome]
        print(f"Risultato parziale (V-P-S): {gioco.giocatore_umano.nome} {score_umano['wins']}-{score_umano['ties']}-{score_umano['losses']} vs {gioco.giocatore_pc.nome} {score_pc['wins']}-{score_pc['ties']}-{score_pc['losses']}")
        
        stampa_riepilogo_match(gioco, risultati, punti_totali, numero_partite_match, i + 1)
        ...
        giocatore_di_mano_corrente = gioco.giocatore_pc if giocatore_di_mano_corrente == gioco.giocatore_umano else gioco.giocatore_umano
        time.sleep(1.5)
    ...
    if vincitore_match:
        ...
        print("Incredibile! Il match è finito in PATTA ASSOLUTA, anche nei punti totali!")
